...
public class PrioritySchedulerTests {
    ...
    public static void main(String[] args) {
        System.out.println("========================================");
        System.out.println("   Priority Scheduler Unit Tests");
        System.out.println("========================================\n");
    ...
    private static void runTest(File file) throws IOException {
        ...
        // Get aging interval
        int agingInterval = input.has("agingInterval") ?
                input.get("agingInterval").getAsInt() : 5;
        ...
        // Get Priority expected output from nested structure
        JsonObject expectedOutput = testCase.getAsJsonObject("expectedOutput");
        JsonObject priorityExpected = expectedOutput.getAsJsonObject("Priority");

        // Run Priority Scheduler
        PriorityScheduler scheduler = new PriorityScheduler(agingInterval);
        SchedulerResult result = scheduler.schedule(processes, contextSwitch);

        // Check results
        boolean passed = checkResult(testName, result, priorityExpected);
